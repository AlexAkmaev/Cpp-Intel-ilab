...
struct Point final {
    ...
    explicit Point(const T& A, const T& B, const T& C) : x(A), y(B), z(C) {}
};
...
struct Vector final {
	...
	explicit Vector(const T& x_, const T& y_, const T& z_) :
        x (x_), y (y_), z (z_) {}
...
struct Triangle final {
	Point<T> pts[3];
	explicit Triangle(const Point<T>& A, const Point<T>& B, const Point<T>& C) : pts {A, B, C} {}
...
struct Sphere final{
	Point<T> centr;
	T R;
	explicit Sphere(const Triangle<T>& t) : centr(Point<T>{(t.pts[0].x + t.pts[1].x + t.pts[2].x) / 3.0f,
                       (t.pts[0].y + t.pts[1].y + t.pts[2].y) / 3.0f,
					   (t.pts[0].z + t.pts[1].z + t.pts[2].z) / 3.0f}),
...
struct Line final {
	...
	explicit Line(Point<T> A, Point<T> B) : fst(A), sec(B) {}
...
struct Plane final {
	Vector<T> n;
	explicit Plane(const Vector<T>& v) : n(v / v.length()) {}
};
